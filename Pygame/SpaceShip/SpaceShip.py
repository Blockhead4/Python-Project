...
#         self.image = pygame.image.load('stealth.png')
#         self.rect = self.image.get_rect()
#         self.rect.x = x - self.rect.centerx
#         self.rect.y = x - self.rect.centery
    ...
    while True:
        ...
        if paused:
            ...
            if random.randint(1, occur_prob) == 1:
                ...
                # if random.randint(1, occur_prob * 10) == 1:
                #     stealth = Stealth(random.randint(30, WINDOW_WIDTH - 30),
                #                 random.randint(30, WINDOW_HEIGHT- 30))
                #     stealths.add(stealth)
            
            draw_text('점수: {}'.format(score), default_font, screen, 80, 20, YELLOW)
            draw_text('워프: {}'.format(warp_count), default_font, screen, 700, 20, BLUE)
            # draw_text('스텔스: {}'.format(stealth_time), default_font, screen, 700, 20, BLUE)
            rocks.update()
            warps.update()
            # stealths.update()
            rocks.draw(screen)
            warps.draw(screen)
            # stealths.draw(screen)

            warp = spaceship.collide(warps)
            # stealth = spaceship.collide(stealths)
            ...
            #     stealth.kill()
            #     stealth_start = time()
            #     if time() - stealth_start <= 2:
            #         stealth_on = True
            #     else:
            #         stealth_on = False

                # 스텔스 아이템 발동, 지속시간 설정해주기
...
def game_screen():
    ...
    draw_text('우주 암석 피하기',
              pygame.font.Font('NanumGothic.ttf', 70),
              screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 3.4, WHITE)
    draw_text('점수: {}'.format(score),
              default_font,
              screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2.4, YELLOW)
    draw_text("마우스 버튼이나 'S'키를 누르면 게임이 시작됩니다.",
              default_font,
              screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2.0, WHITE)
    draw_text("게임을 종료하려면 'Q'키를 누르세요.",
              default_font,
              screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 1.8, WHITE)
